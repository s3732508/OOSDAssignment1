...
public class Game extends AbstractSubject implements java.io.Serializable {
	...
	public List<Piece> createPieces(Point[] positions, PieceAbstractFactory factory) {
		...
		// Tanks
		for (int i = this.getBoard().getSize(); i <= (this.getBoard().getSize() - 1) * 2; i++) { // 3to4 for small, 4to6
																									// for big
	...
	public void executeAttack(Tile tile) {
		...
		if (valid) {
			...
			System.out.println(
					"Attacker: " + attacker.getClass().getSimpleName() + " -- Attack: " + attacker.getAttack());
			System.out.println("VS.");
			System.out
					.println("Defender: " + target.getClass().getSimpleName() + " -- Defense: " + target.getDefence());
